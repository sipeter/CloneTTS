...
切换到 **“高级设置”** 界面并打开首行的 **“本地 HTTP API 服务”**。将记录下的接口地址（如 `http://127.0.0.1:8080/api/tts?text={txt}`）完整填入外部阅读软件（如 Legado）的网络发音设置中即可。
...
Switch over to the **"Engine Settings"** tab and slide on the topmost switch for the **"Local HTTP API Service"**. Key in the API address string displayed just below into external reading apps' web-TTS configurations (e.g., `http://127.0.0.1:8080/api/tts?text={txt}`). 
